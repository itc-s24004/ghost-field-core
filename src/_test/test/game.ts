...
export async function dev_game() {
    ...
    game.on("", "onDrawCard", (ev) => {
        console.log(`${game.getPlayerIndex(ev.player)}はカードをドローした`);
        console.log(`引いたカード: ${ev.drawnCard.name}`);
        if (ev.removedCard) console.log(`捨てたカード: ${ev.removedCard.name}`);
    }).on("", "onUseCard", (ev) => {
        ...
        if (action.isMiss) console.log(`${game.getPlayerIndex(action.src)}の攻撃は${game.getPlayerIndex(action.target)}に命中しなかった！`);
    }).on("", "onExchange", (ev) => {
    ...
    while (true) {
        ...
        if (game.currentAction) {
            ...
            game.next(selectedCards, game.currentPlayerIndex, 
                baseCard?.canUseOptions ? await inputSelectOptions(game.currentPlayer!) : {}
            )
            ...
            const baseCard = selectedCards[0];
            const index = baseCard ? await inputSelectIndex("使用対象を選択してください",
                new Array(game.playerCount).fill(0).map((_, i) => i === game.currentPlayerIndex ? `${i} (自分)` : `${i}`)
            ) : game.currentPlayerIndex
            game.next(selectedCards, index,
                baseCard?.canUseOptions ? await inputSelectOptions(player) : {}
            );
